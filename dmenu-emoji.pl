...
# Fun fact:  In a previous life this used to be the shell one-liner from a 2018
# Luke Smith video (https://youtu.be/UCEXY46t3OA).
...
my @dmenu_cmd = qw(dmenu -i -p emoji -l 10 -fn sans:size=16);
my $dmenu_pid = open2 my $dmenu_out, my $dmenu_in, @dmenu_cmd or die $!;
binmode $dmenu_out, ':encoding(UTF-8)';
binmode $dmenu_in, ':encoding(UTF-8)';
print $dmenu_in $EMOJIS;
close $dmenu_in;
my $answer = <$dmenu_out>;
close $dmenu_out;
waitpid $dmenu_pid, 0;
...
if (defined $answer && $answer ne '') {
    my ($emoji) = split ': ', $answer, 2;
    die 'shouldnt happen' if (!defined $emoji);
    my $display_server = $ENV{'XDG_SESSION_TYPE'} || 'x11';
    my @clip_cmd;
    if ($display_server eq 'wayland') {
        @clip_cmd = qw(wl-copy);
    } else {
        @clip_cmd = qw(xclip -selection clipboard);
    }
    my $clip_pid = open my $clip_in, '|-:encoding(UTF-8)', @clip_cmd or die $!;
    print $clip_in $emoji;
    close $clip_in;
    if ($? == 0) {
        system qw(notify-send -u low), "'$emoji' copied to clipboard";
        exit $?>>8 if ($?);
    } else {
        system qw(notify-send -u low), "coudn't copy to clipboard: error $?";
        exit $?>>8 if ($?);
    }
}
